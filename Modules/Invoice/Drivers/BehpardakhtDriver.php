...
class BehpardakhtDriver extends PayDriver
{
    public function make($amount, string $callback): GatewayMakeResponse
    {
        /** @var Payment $shetabitPayment */
        $shetabitPayment = \Shetabit\Payment\Facade\Payment::via('behpardakht')->callbackUrl($callback);
        $settings = app(CoreSettings::class);
        foreach ($settings->get('invoice.active_drivers.behpardakht.config') as $key => $value) {
    ...
    {
        try {
            $shetabitPayment = \Shetabit\Payment\Facade\Payment::via('behpardakht');
            $settings = app(CoreSettings::class);
            foreach ($settings->get('invoice.active_drivers.behpardakht.config') as $key => $value) {
                $shetabitPayment->config($key, $value);
            }
    ...
    {
        return $request?->input('RefId') ?? request('RefId');
    }
